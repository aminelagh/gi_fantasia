...
@section('content')
  ...
  <div class="row">
    <div class="col-md-12">
                  ...
                  <div class="form-group has-feedback">
                    <select class="form-control selectpicker show-tick" data-live-search="true" name="id_zone" id="filter_id_zone">
                      ...
                      @foreach ($zones as $item)
                        <option value="{{ $item->id_zone }}" {{ isset($selected_id_zone) && $selected_id_zone == $item->id_zone ? 'selected' : ''  }}>{{ $item->libelle }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
      ...
      <form role="form" name="formValidateInventaires" id="formValidateInventaires" method="POST" action="{{ route('controleur') }}">
        @csrf
            ...
            @if(isset($data) && $data != NULL )
              @foreach($data as $item)
                <tr>
                  ...
                  <td align="center">
                    <input type="checkbox" name="valide[{{ $loop->iteration }}]" value="{{ $item->id_inventaire }}" {{ $item->validated_by != null ? "checked title=Valide" : "title=non-valide" }}>
                  </td>
                  <td align="center">
